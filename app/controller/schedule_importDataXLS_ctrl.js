...
module.exports = (req, callback) => {
	...
	async.waterfall([
		(next) => {
			...
			validator.schedule_importDataXLS_val(val, (err, value) => {
		...
		(value, next) => {
			/**
			 * 2. cek department, employee id tersebut memang berada di department terkait, 
			 *    cek field department di table emp
			 * @callback Requester~requestCallback
			 * @param {string} value
			 * @param {string} err		The callback that handles the response error.
			 * @param {string} res		The callback that handles the response succes.
			 */
			let emp_id = [];
			let dept = value.department.join();
			value.data.map((a, index) => emp_id[index] = value.data[index].employee_id);
			procedure.schedule_import_check_emp_dept_pro({emp: emp_id, department: dept}, (err, res) => {
				if (err) return next(true, res);
				try {
					if (res[0][0].status == 'Error') return next(true, res[0][0].message);
					return next(null, value);
		...
		(value, next) => {
			/**
			 * 3. cek shiftcode hanya yg terdaftar di master workshift
			 * 
			 * @callback Requester~requestCallback
			 * @param {string} value
			 * @param {string} err		The callback that handles the response error.
		...
		(value, next) => {
			 ...
			 * @param {string} res		The callback that handles the response succes.
			 */
			procedure.schedule_import_insert_to_tmp_pro(value.ready_insert, (err, res) => {
				if (err) return next(true, res);
				try {
					return next(null, value);
				} catch (error) {
        ...
        (value, next) => {
			/**
			 * 1. get emp name setiap row dari value.data[index].employee_id 
			 * 
			 * @callback Requester~requestCallback
			 * @param {string} value
			 * @param {string} err		The callback that handles the response error.
			 * @param {string} res		The callback that handles the response succes.
			 */
			procedure.schedule_import_get_emp_pro(value, (err, res) => {
				if (err) return next(true, res);
				try {
					return next(null, {...value, response_val: res});
				} catch (error) {
					return next(true, 'Please check and try again.')
				}
			});
		...
		(value, next) => {
			...
			try {
				let shift_id = null;
				let data_color = {};
				for (let i = 0; i < value.response_val.length; i++) {
					init_column.map((el, index) => {
						if (value.response_val[i][el]) {
							shift_id = value.workshift_data.find(element => element.shift_code == value.response_val[i][el]);
							data_color['color' + el] = shift_id.color;
						} else {
							data_color['color' + el] = '';
						}
					});
					ready_response.push({
						...value.response_val[i],
						...data_color
					});
					// if last looping then send data to next
					if ((i + 1) == value.response_val.length) {
						return next(null, { ...value, ready_response });
					}
				}
			} catch (error) {
